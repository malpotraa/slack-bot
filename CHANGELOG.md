...
- **Conversational Google Ads analyst.** The DM agent answers free-form Google Ads performance questions across every campaign type (Search, Shopping, Performance Max, Demand Gen, Video, Display) and at every level — account, campaign, ad group, keyword, search term, ad, asset group, product. READ-ONLY: it fetches and explains, never changes the account, and **never makes recommendations** — it reports and describes only, even when asked for advice. Answers are framed in cost / conversions / cost-per-conversion (ROAS only when the user explicitly asks). The new `get_google_ads_data` tool resolves the account (asking to disambiguate when needed) and the agent narrates a streamed answer that leads with total spend and the date range.
- **Surface-first, drill-deeper.** The first question returns an account overview — a per-channel rollup and a top-campaigns table. The user digs into a campaign by tapping a `🔍` button on the table or just asking. Drill-downs are channel-aware: keywords / search terms for Search, asset groups for Performance Max, products for Shopping, ad groups + ads for Demand Gen / Video / Display — and the tool redirects requests that don't apply (e.g. keywords on Performance Max).
- **Tables every answer, charts on request.** Each answer posts a monospace table — Campaign, Cost, Conversions, CPC, Cost/conv, conversion rate, CTR (ROAS swapped in only when the user asks; percent-change columns only when the user asks). Charts (matplotlib PNGs of daily metrics over a chosen window, rendered off the event loop) render only when the user explicitly asks for one.
- **Per-thread Google Ads caches.** One 90-day account snapshot (daily campaign metrics, the same 30 days a year ago, impression share, budgets) is cached in-process per Slack thread for 20 minutes; drill-downs reuse it and cache their own breakdown, so follow-ups are fast.
- **Thread-sticky Google Ads account.** Once an account is established in a thread (by the agent or by `/kpi`), follow-up questions reuse it without re-specifying. A question started in a new thread re-verifies the account.
- **New modules** `app/integrations/google_ads_snapshot.py` (surface tier), `app/integrations/google_ads_drill.py` (channel-aware drill-downs), `app/formatters/ads_blocks.py` (tables + drill buttons) and `app/formatters/ads_charts.py` (PNG charts). As with `/kpi`, all metric math is deterministic Python; the model only narrates a precomputed fact pack.
...
- **Cloud Run secret mapping includes Google Ads runtime config:** `GOOGLE_ADS_DEVELOPER_TOKEN`, `GOOGLE_ADS_LOGIN_CUSTOMER_ID`, and `GOOGLE_ADS_API_VERSION=v24`.
...
- **Google Ads API version raised v22 → v24** — Google moved to a monthly release cadence and v22 is near sunset.
- **`/kpi` flows into the conversational analyst.** A successful `/kpi` pull marks its thread as a Google Ads thread, so questions typed under a `/kpi` report are answered for that account without re-specifying it.
- **`run_agent_turn` returns tool-emitted messages and gained a tool-start hook** — a tool can post a ready-made card (the Google Ads table + charts) in-thread without routing raw Block Kit back through the model, and the placeholder can show "Pulling Google Ads data…" while a slow tool runs.
- **`dispatch_tool` accepts the Slack `channel_id` and `thread_ts`** so per-thread tools — the Google Ads snapshot and drill caches — can key on the conversation.
- **`matplotlib` added as a runtime dependency** for server-side chart rendering.
- **Phoenix tracing redaction is now structural, not all-or-nothing.** With `TRACE_SENSITIVE_DATA=false` (default), user prompts, tool names, tool call args and the assistant's reply are traced in full; tool *responses* (Calendar / Wrike / Google Ads data) are traced as shape + types only — dict keys and list counts are kept, leaf values become `<str:N>` / `<int>` / `<float>` / `<bool>` tokens. This keeps traces useful for AI-observability work and evals without exporting customer data values. `TRACE_SENSITIVE_DATA=true` still traces everything raw and enables Anthropic SDK auto-instrumentation.
- **In-process caches are now bounded.** A shared `app/utils/cache.py` (`TTLCache` — LRU + TTL — and `KeyedLocks`) replaces the unbounded module dicts behind the calendar-event prefetch, Google Ads account / snapshot / drill caches, and per-thread history locks, so a long-lived instance can't accumulate them without limit. The calendar credential/service caches gained a safety cap.
- **Concurrent Google Ads pulls are de-duplicated.** Snapshot and drill fetches now take a per-key lock (singleflight) — two near-simultaneous misses for the same thread share one 90-day pull instead of both hitting the API.
- **Cloud Run memory raised 512 MiB → 1 GiB,** and concurrent matplotlib chart renders are capped by a semaphore. The chart dependency raised the memory floor; 512 MiB left no headroom against an OOM kill, which would restart the single Socket Mode instance.
- **Conversation history is retained 60 days, then purged** by the startup cleanup — agent-thread sessions previously had `expires_at=None` and accumulated indefinitely.
- **The bot's own Slack identity is cached** (1 h TTL) instead of re-fetched with `auth.test` + `users.info` on every `/goodmorning` mention search.
- **System prompt split into a cached static block + an uncached per-turn block.** It previously interpolated the current time (to the minute) into the single cache-marked block, so the cache key changed every turn and the whole prompt was re-tokenized at full price. The static rules now cache cleanly; only the small user / date / timezone / working-hours block is uncached.
- **The static prompt and tool definitions cache with a 1-hour TTL** (was the 5-minute default) so Slack follow-ups landing more than 5 minutes apart still hit the cache.
- **The Google Ads fact pack sent to the model is a slim, labeled digest.** The raw 90-day daily series and unused per-campaign windows are dropped from the default model view (they still drive the deterministic tables + charts); each campaign is a labeled record with its last-30 metrics and period-over-period deltas. Conversion value and ROAS are omitted unless the user asked for ROAS.
- **Google Ads comparison slices are on-demand and scope-aware.** The default digest stays lean with last-30 + month-over-month context. The agent can request `extras` only on explicit intent: `wow` for week-over-week, `yoy` for year-over-year, `day_of_week` for weekday patterns, and `90d` for a deterministic 90-day summary. Account questions receive account-level slices; selected-campaign questions receive campaign-level slices computed from that campaign's cached daily series/windows. The full cached snapshot still feeds every slice, so this adds no Google Ads API round trip and only spends Anthropic tokens when the user asks for that view.
- **Per-turn cost is recorded in Phoenix** as `llm.cost_usd`, computed from the model and the input / output / cache token counts — so spend is filterable by user, command and flow.
- **Tool results are JSON-encoded compactly,** and the agent's `max_tokens` cap is 1200 (was 2048) — a guard against runaway output, not a normal-case saver.
- **Conversation history compacts sooner** — after 8 turns instead of 12, keeping 4 verbatim — capping the history re-sent on every turn.
- **The Google Ads agent no longer drills unless asked.** A question about one campaign's metrics or trend ("how is campaign X doing", "what's its conv rate") is answered from the account overview — the model digest now carries every campaign's period-over-period deltas, so no drill is needed. A drill scope (ad groups / keywords / ads / products / asset groups) runs only when the user explicitly asks to break the campaign down.
- **Charts are strictly opt-in and relevant.** A chart renders only on an explicit "chart" / "graph" / "plot" request — "trend" or "over time" no longer trigger one. When a chart is asked for it reflects the metric in the question, and a request for one campaign's metric over time renders a per-campaign trend line (a per-campaign daily series is kept tool-internally for this — it's not sent to the model). A per-campaign chart request posts just that chart, not the whole account table.
- **The Google Ads agent re-fetches on every question.** It calls `get_google_ads_data` for every Ads question — including follow-ups like "graph it" — and answers only from that turn's fact pack. The fact pack is never kept in conversation history, so answering a follow-up from memory previously produced fabricated figures; a deterministic per-campaign trend summary is also handed to the model so it narrates direction from labeled facts, not from the posted chart.
- **`/kpi` campaign notes are back, now reconciliation-guarded.** Each campaign and the account total can again carry a one-line model-written explanation under its WOW/MOM rows. The model is handed a fact pack in which every figure it may quote — cost, conversions, CPC, conversion rate, CTR, and every period-over-period percentage, at both campaign and keyword-driver level — is precomputed in deterministic Python (`app/llm/kpi_summary.py`); the prompt forbids the model from computing anything. `explain_kpi_report` then runs a numeric reconciliation pass: any note containing a `$`, `%`, or decimal figure that does not trace back to that item's fact pack within a rounding tolerance is dropped (and logged). A dropped note degrades to the plain deterministic rows — never a wrong number. Notes are written by `EXTRACTION_MODEL` (Claude Haiku 4.5). Conversion rate stays defined as `conversions / clicks`.
...
- *Why bring the `/kpi` notes back after removing them?* An earlier pass removed the model-written notes outright because the model was doing its own arithmetic — deriving percentages, CPC, and conversion rate from raw counts — which produced authoritative-looking figures that couldn't be trusted next to the correct deterministic rows. Dropping the feature lost a genuinely useful summary. The durable fix is to remove the model's *need* to compute: the fact pack now carries every number precomputed, so narration is pure quoting, and a deterministic reconciliation guard deletes any note whose numbers don't trace back to it. Numeric hallucination is effectively eliminated — a bad note is dropped, not shown, so the worst case is the plain rows. The honest residual is mild causal framing on keyword drivers (e.g. "visible driver"), which the prompt keeps hedged and which is bounded because the model only ever sees real drivers. Haiku stays the notes model — cheap and fast — because the guard, not the model, is the reliability boundary.
...
- *Why surface-first with tiered, channel-aware fetching?* Reportable resources differ sharply by campaign type — Performance Max has no keywords or ad groups; Demand Gen has ad groups and ads. Pulling every level for every campaign upfront would be enormous and slow. One 90-day account snapshot answers account/campaign questions and is re-sliced deterministically; a campaign drill then pulls only that campaign's channel-appropriate breakdown.
- *Why no approval card for the Google Ads analyst?* It is strictly read-only, so there is nothing to gate for safety. Dropping the card and streaming the answer token-by-token removes both the click and the blank post-click wait the card-based flow had.
- *Why keep the LLM out of the metric math here too?* Same reason as `/kpi`: spend and conversion arithmetic must be reproducible. Deterministic code computes every total, ratio, and delta; the model only narrates the precomputed fact pack.
- *Why make the account sticky on the thread?* A Google Ads analysis is a conversation — users drill in with follow-ups. Re-asking which account every turn would be noise. The account is persisted on the thread's session row and re-verified only when a new thread starts.
- *Why reporting-style with no recommendations, and cost/conv by default?* The analyst is an observability / reporting surface — it states what the numbers and movements are (with exact percentages) and stops. It never advises actions, even when asked, so its output is predictable and safe to build evals on. Cost-per-conversion is the default lens because conversion-value tracking is inconsistent across accounts, which makes ROAS misleading unless the user has opted into it.
- *Why the no-drill-unless-asked change?* A narrow question — "what's campaign X's conv rate and trend" — was triggering a full ad-group + keyword drill plus an unrequested chart, because the slimmed digest lacked per-campaign trend deltas so the agent escalated to a drill to get them. The digest now carries those deltas; drills and charts are treated as explicit, separate requests.
- *Why the token-cost changes?* The agent's variable cost is Anthropic usage. Two structural fixes dominate: the system-prompt cache was silently defeated by a per-minute timestamp baked into the cached block (now split out), and the Google Ads tool handed the model the entire raw snapshot when it only needs a digest to narrate — deterministic code already owns the tables and charts. Together they take a typical Ads turn from a full-price prompt + ~10K-token fact pack to a cached prefix + ~1.8K-token digest. `llm.cost_usd` per turn makes the remaining spend measurable.
- *Why labeled digest records instead of a columnar array?* An earlier pass encoded the campaign digest as field-names-once + positional rows to save tokens, but that made the model count columns to find a value — it misread, and on one turn fabricated a campaign's conversion rate. Labeled per-campaign records cost ~800 more tokens (still ~80% off the raw snapshot) and remove the misread surface entirely. Correct numbers outrank token count for a reporting tool.
- *Why bound the caches and add singleflight now?* The instance is always-on and only restarts on deploy, so any unbounded module-level dict is a slow memory leak — and the new Google Ads features added several. `TTLCache` / `KeyedLocks` cap them centrally; the singleflight lock stops a user double-send from triggering two expensive 90-day Ads pulls. Memory went to 1 GiB in the same pass because matplotlib raised the resident floor and an OOM kill on the single instance is a visible outage.
- *Why structural redaction instead of the binary trace flag?* The old flag redacted whole payloads to `[redacted]`, which made traces useless for evaluation; tracing everything raw exports customer spend / calendar / task values to Phoenix. Masking only the leaf values of tool responses keeps the eval-relevant signal — the prompt, the tool and args chosen, the response *shape*, and the reply — while the actual values stay out. Anthropic SDK auto-instrumentation remains off by default because those spans carry the raw tool results inside the message history.
